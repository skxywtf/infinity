...
const MacroLineChart = dynamic(() => import('@/components/macrodata/MacroLineChart'), { ssr: false, loading: () => <p style={{ color: '#888', padding: '20px' }}>Loading chart data...</p> });
...
export default function MacroPage() {
  ...
  const [market, setMarket] = useState<any>({ spy:{price:'---',change:'0.00%',pos:true}, ief:{price:'---',change:'0.00%',pos:true}, uup:{price:'---',change:'0.00%',pos:true}, btc:{price:'---',change:'0.00%',pos:true}, gold:{price:'---',change:'0.00%',pos:true} });
  const [news, setNews] = useState<any[]>([]);

  useEffect(() => {
    fetch('/api/tabs').then(r=>r.json()).then(data=>{setMetadata(data);if(data.length>0)setActiveTab(data[0].tab_name);}).catch(console.error);
    fetch('/api/latest/BEA_REAL_GDP').then(r=>r.json()).then(d=>{const v=parseFloat(d.value);setLatestGDP(isNaN(v)?null:v);}).catch(console.error);
    fetch('/api/data/USREC').then(r=>r.json()).then(data=>{setRecession(data.map((d:any)=>({time:d.date,value:parseFloat(d.value)})));}).catch(console.error);
    const fetchMarket = async (symbol: string) => { try { const r=await fetch('/api/market/'+symbol); if(!r.ok) throw new Error('bad'); return await r.json(); } catch { return {price:'---',change:'0.00%',pos:true}; } };
    const fetchYahooNews = async () => { try { const r=await fetch('/api/news'); const j=await r.json(); setNews(j.data||[]); } catch { setNews([]); } };
    (async()=>{ const [spy,ief,uup,btc,gold]=await Promise.all([fetchMarket('SPY'),fetchMarket('IEF'),fetchMarket('UUP'),fetchMarket('BTC-USD'),fetchMarket('GC=F')]); setMarket({spy,ief,uup,btc,gold}); fetchYahooNews(); })();
  }, []);

  const dbTabs = Array.from(new Set(metadata.map((m:any)=>m.tab_name))).filter(tab=>!HIDDEN_TABS.includes(tab));
  const allTabs = [...dbTabs, ...SPECIAL_TABS.filter(t=>!dbTabs.includes(t))];
  ...
  return (
    <main style={{maxWidth:'1800px',margin:'0 auto',padding:'20px',backgroundColor:'#000',minHeight:'100vh',color:'white',fontFamily:'sans-serif'}}>
      ...
      <button className="mobile-toggle" onClick={()=>setSidebar(!isSidebarOpen)}>{isSidebarOpen?'Hide Terminal Menu ▲':'Show Terminal Menu ▼'}</button>
      <div className="terminal-grid">
        <div className={'sidebar-container '+(isSidebarOpen?'open':'')}>
          ...
          <aside className="card" style={{background:'#0b0f0f',border:'1px solid #1b2226',borderRadius:'16px',padding:'20px',textAlign:'center',marginTop:'auto'}}>
            ...
            <div style={{fontSize:'13px',color:'#aaa',padding:'10px 0',lineHeight:'1.5'}}>Advertisement Space Available<br/><span style={{fontSize:'11px',opacity:0.6}}>(Contact sage@worldtradefactory.com)</span></div>
          </aside>
        </div>
        <section style={{display:'flex',flexDirection:'column',gap:'20px',minWidth:0}}>
          <div style={{display:'flex',gap:'10px',borderBottom:'1px solid #1b2226',paddingBottom:'10px',overflowX:'auto'}}>
            {allTabs.map(tab=>(
              <button key={tab} onClick={()=>setActiveTab(tab)} style={{background:activeTab===tab?'#1b2226':'transparent',color:activeTab===tab?(SPECIAL_TABS.includes(tab)?'#d4af37':'#fff'):(SPECIAL_TABS.includes(tab)?'#d4af3799':'#888'),border:SPECIAL_TABS.includes(tab)?'1px solid #1b222666':'none',padding:'10px 20px',borderRadius:'8px',cursor:'pointer',fontWeight:600,transition:'0.2s',whiteSpace:'nowrap'}}>{tab}</button>
            ))}
          </div>
          <div style={{display:'flex',flexDirection:'column',gap:'40px'}}>
            ...
            {activeTab==='Positioning' && activeCharts.length===0 && (
              <div style={{background:'#0b0f0f',border:'1px solid #1b2226',borderRadius:'16px',padding:'40px',textAlign:'center',color:'#444',fontSize:'13px'}}>
                No positioning data yet — run <code style={{color:'#d4af37'}}>ingest_spec_series.py</code> to populate COT data.
              </div>
            )}
            ...
            {activeTab==='Positioning' && activeCharts.map((chart:any)=>(
              <div key={chart.series_id} className="card chart-wrapper" style={{background:'#0b0f0f',border:'1px solid #1b2226',borderRadius:'16px',padding:'20px',overflow:'hidden'}}>
                ...
                <div style={{height:'calc(100% - 40px)'}}><MacroLineChart seriesId={chart.series_id} recessionData={recessionData}/></div>
              </div>
            ))}
            {!SPECIAL_TABS.includes(activeTab) && activeCharts.length===0 && activeTab!=='' && <p style={{color:'#888'}}>Loading charts...</p>}
          </div>
        </section>
      </div>
