...
export async function GET(request: Request) {
  ...
  try {
    const res = await fetch(`${BACKEND}/api/brief/generate`, { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ trigger_series: "cron_daily", brief_type: "daily" }), signal: AbortSignal.timeout(55000) });
