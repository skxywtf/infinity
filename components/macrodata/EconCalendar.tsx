...
interface CalendarEvent { id: string; event_date: string; country: string; indicator: string; importance: number; prior: number|null; consensus: number|null; actual: number|null; surprise: number|null; source: string; }
const IMPORTANCE_LABEL: Record<number,{label:string;color:string}> = { 3:{label:'HIGH',color:'#e05c5c'}, 2:{label:'MED',color:'#f59e42'}, 1:{label:'LOW',color:'#4a6868'} };
function fmt(v: number|null) { if (v===null||v===undefined) return '—'; return v.toFixed(2); }
function surpriseColor(v: number|null) { if (v===null) return '#666'; if (v>0) return '#4caf82'; if (v<0) return '#e05c5c'; return '#888'; }
function groupByDate(events: CalendarEvent[]) {
  const groups: Record<string,CalendarEvent[]> = {};
  events.forEach(e => { const k = e.event_date.substring(0,10); if (!groups[k]) groups[k]=[]; groups[k].push(e); });
  return groups;
}
...
export default function EconCalendar() {
  ...
  if (needsMigration) return (
    <div style={{ background: '#0b0f0f', border: '1px solid #1b2226', borderRadius: '16px', padding: '40px', textAlign: 'center' }}>
      <div style={{ fontSize: '18px', fontWeight: 700, color: '#fff', marginBottom: '16px' }}>Economic Calendar</div>
      <div style={{ fontSize: '13px', color: '#444', lineHeight: '1.8' }}>
        Database tables not yet created.<br/>
        <code style={{ color: '#d4af37', fontSize: '12px' }}>python -m macro_scripts.migrate_spec</code><br/>
        <code style={{ color: '#d4af37', fontSize: '12px' }}>python -m macro_scripts.ingest_spec_series</code>
      </div>
    </div>
  );
  ...
  return (
    <div style={{ background: '#0b0f0f', border: '1px solid #1b2226', borderRadius: '16px', padding: '20px' }}>
        ...
        <div style={{ display: 'flex', background: '#1b2226', borderRadius: '6px', padding: '2px' }}>
          {(['upcoming','history'] as const).map(t => (
            <button key={t} onClick={() => setTab(t)} style={{
              padding: '5px 12px', fontSize: '10px', border: 'none', borderRadius: '4px', cursor: 'pointer',
              textTransform: 'uppercase', letterSpacing: '1px', fontWeight: 700,
              background: tab===t ? '#333' : 'transparent', color: tab===t ? '#fff' : '#666',
            }}>{t}</button>
          ))}
        </div>
      </div>
      ...
      {!loading && sortedDates.length === 0 && (
        <div style={{ color: '#333', fontSize: '13px', padding: '20px 0', textAlign: 'center' }}>
